...
class Sesion
{
    ...
    private function __construct($idUser,  $signIn, $nameUser, $dtIn, $dtOut = null, $idSesion = null)
    {
     ...
     * @return null|PDO
     */
    public static function sesion( $idUser, $signIn, $nameUser, $dtIn, $dtOut = null)
    {
        if (!isset(self::$instancia)) {
            $c = __CLASS__;
            self::$instancia = new $c( $idUser, $signIn, $nameUser, $dtIn, $dtOut = null);
        }
   ...
   /* private function setConexion()
    ...
    public function gestionarSesiones($sesion)
    {
 ...
 //       echo strtotime(date('Y-m-d H:i:s')) - strtotime($this->getDtIn())." ".(date('Y-m-d H:i:s'))." - ". ($this->getDtIn());
        if ($this->getStatusSesion($this->getIdUser())) {
             $hora = $this->getDtIn();
